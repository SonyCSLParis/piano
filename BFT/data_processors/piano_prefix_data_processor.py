...
class PianoPrefixDataProcessor(DataProcessor):
    ...
    def preprocess(self, x):
        ...
        is_start_token = (
            x[:, :, 0] == self.start_tokens[0].unsqueeze(0).unsqueeze(0).repeat(
                batch_size, num_events))
        ...
        placeholder_duration_token = cuda_variable(torch.Tensor([
            self.dataloader_generator.dataset.value2index['time_shift']
            [find_nearest_value(
                self.dataloader_generator.dataset.time_table_time_shift, pd.item())]
            for pd in placeholder_duration
        ])
        )
        
        # placeholder is batch_size, 1, 4
        placeholder = self.placeholder_symbols.unsqueeze(0).unsqueeze(0).repeat(batch_size, 1, 1)
        placeholder[:, 
                    0, 
        ...
        for (b, m, a, p_duration, p_duration_token, c_start_token, c_end_token,
                ...
                new_middle = torch.cat(
                    [ m[start_token_l - self.num_events_before:], # removes pad
                        ...
                        self.pad_tokens.unsqueeze(0).repeat(
                            start_token_l - self.num_events_before + 1, 1), # number_of_removed events + 1
                        ], dim=0)
            ...
            # == if START token is in "after"                             
                ...
                # no END!
                new_after = torch.cat([
                    a[start_token_l - (self.num_events_before + num_events_middle):], # trim leading PAD
                    self.pad_tokens.unsqueeze(0).repeat(
                        (start_token_l - (self.num_events_before + num_events_middle), 1) # put trimmed PAD ad the end of the sequence
                    )
                ], dim=0                        
                )
        ...
        y = torch.cat(
            [
                ...
                new_after,
                new_middle,
                self.pad_tokens.unsqueeze(0).unsqueeze(0).repeat(batch_size, remainder_num_events, 1)
            ], dim=1)
